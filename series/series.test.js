const { getGenres, generateSeriesObj } = require("./logic");
...
describe("Testing getGenres returns array of genres for a single series", () => {
  test("Should return array when given a series object", () => {
    const actual = getGenres(obj);
    const expected = ["Action", "Sci-Fi"];

    expect(actual).toEqual(expected);
  });

  test("Should return error msg when given an undefined or null object", () => {
    let newObj = null;
    const actual = getGenres(newObj);
    const expected = "The object is undefined or null !!";

    expect(actual).toEqual(expected);
  });

  test("Should return error msg when given an empty object", () => {
    let newObj = {};
    const actual = getGenres(newObj);
    const expected = "The series is not categorized .";

    expect(actual).toEqual(expected);
  });
});

describe("Testing generateSeriesObj returns new clone from the fiven object with less keys", () => {
  test("Should return new array of filtered objects when given an array of objects", () => {
    const actual = generateSeriesObj([obj]);
    const expected = [{
        "title": "Cowboy Bebop",
        "duration": "24 min per ep",
        "img": {
        ...
        "trailer": "https://www.youtube.com/watch?v=qig4KOK2R2g",
        "detailesURL": "https://myanimelist.net/anime/1/Cowboy_Bebop",
        "id": 1
    }];
    expect(actual).toEqual(expected);
  });

  test("Should return error msg when given an undefined or null array", () => {
    let newObj = null;
    const actual = generateSeriesObj(newObj);
    const expected = "No data given !!";;

    expect(actual).toEqual(expected);
  });

  test("Should return error msg when given an empty array", () => {
    let newObj = [];
    const actual = generateSeriesObj(newObj);
